...
**⚠️ The naive fix un-delivers the phone.** The Wear APK shares `applicationId` with the phone app, so Play sees one app whose track release must list both version codes. A second, independent `upload_to_play_store` call creates a new release on the same track and the phone's version code silently drops out of it — a plausible-looking result that is wrong, which is the Story 16.7 failure class. Compounding it, `supply` accepts an APK **or** an AAB but not both — `:apk`/`:apk_paths` are declared `conflicting_options` with `:aab`/`:aab_paths` — and the phone ships an AAB while the Wear app ships an APK, so they cannot travel in one invocation. The mechanism that works is a second call with `skip_upload_aab` plus `version_codes_to_retain`.
...
- **AC3 — one Play release carries both version codes, and the ordering is enforced.** Phone AAB first; Wear APK second, in the same job, with `skip_upload_aab: true` and `version_codes_to_retain` carrying the phone's `versionCode`, so the resulting track release lists both. Not a parallel job — two jobs race the same Play track edit and one silently loses. The reason for each constraint is written into the Fastfile, not left to review memory.
