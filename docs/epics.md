...
**✅ Dependency discharged — PR #218 merged as `fe0febc` (2026-09-03), and this story is baselined on it.** `wear_play_track(phone_track)` (`fastlane/Fastfile:372-378`) reads `ENV['WEAR_PLAY_TRACK']` and only falls back to `wear:<phone_track>`; `available_play_tracks` lists the app's real tracks on `Track not found`. So the nightly needs **no Fastfile change** for the track — it sets `WEAR_PLAY_TRACK: wear:qa` in its own workflow env. Only three Wear track names are well known ([developers.google.com/android-publisher/tracks](https://developers.google.com/android-publisher/tracks)): `wear:production`, `wear:beta` (open testing) and **`wear:qa` — internal testing, literally named "qa", not "internal"**; `wear:internal` does not exist. **✅ This app's real Wear track inventory, confirmed by ifero 2026-09-03:** the Wear form factor has **`qa`** (internal testing) and **`alpha`** (the current closed test). So this story's destination `wear:qa` exists — the draft's highest-risk item, now settled — and `wear:alpha` exists too, as a hand-created closed track literally named "alpha", exactly the shape Play's docs describe for anything outside the three well-known names. **That retires an earlier draft's claim that Wear delivery was still broken on the release lanes:** the derived fallback is correct for the RC lane, which therefore needs no override. rc.21's `Track not found: wear:alpha` was true when it happened; the track was created afterwards. The derivation is still not a general rule, which is why the override must stay — the phone's `internal` has no counterpart, because the Wear internal track is named `qa`, and that single gap is the whole reason this story sets `WEAR_PLAY_TRACK`.
...
- **AC7 — Android goes to Play `internal`, and Wear to `wear:qa`.** Not `wear:internal`, which does not exist, and not `wear:` + the phone's track name, which rc.21 disproved. The reason an override is needed is narrow and worth stating exactly: this app's Wear form factor has `qa` and `alpha`, so the phone's `alpha` **does** derive correctly, but the phone's `internal` derives `wear:internal`, which does not exist — the Wear internal track is named `qa`. The mechanism is already on `main` (#218): set `WEAR_PLAY_TRACK: wear:qa` in the workflow env and `wear_play_track` uses it verbatim. **Do not add a second override, a lane parameter or a `WEAR_TRACK_PREFIX` variant** — `Fastfile:369-370` names the env var as the sanctioned seam. Confirm `available_play_tracks`' diagnostic is reached from the nightly lane rather than assuming it.
...
- **AC12 — verified on real runs, recorded in the Dev Agent Record.** The cron fires; a `[skip ci]`-only night skips both platforms; a real-change night ships both; TestFlight carries the Apple Watch; Play `internal` shows `4_000_000 + N`; **`wear:qa` shows `5_000_000 + N` and installs on a paired device** (highest risk — whether a `qa` Wear track exists here is unverifiable from the repo, and rc.21 already burned an RC on the analogous assumption); both baseline tags advanced and triggered no `v*` workflow.
