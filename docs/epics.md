...
**Platform:** iOS, and **this card only** — every other card has always scanned fine, and a Samsung (Android) device decodes this one via both the camera and the image path. So neither the shared JS pipeline nor the iOS image path is broadly broken. Apple **Vision** is not the limitation either: measured against macOS Vision with the app's exact symbology set, the payload decodes at every leading digit 0–9, at 1 px per module, with a 1-module quiet zone, under low-quality JPEG, from a **194 × 40 px** image, and from a faithful reconstruction of the whole card. With the bars confirmed genuine (Android returned the correct value) and the input confirmed to be a synthetic graphic rather than a photo, the **leading hypothesis is that the image never loads on iOS** — `UIImage(contentsOfFile:)` returning `nil` rejects with `INVALID_IMAGE`, the bare `catch {}` swallows it, and the user is told "No barcode found in this image", which would be false. That makes the observability work the diagnostic that names the bug, not merely polish.

**Acceptance Criteria:**

- **The input is classified on a Mac before any device session.** The actual failing file is run through a zero-device macOS Vision harness (Swift source embedded in the story) whose outcome selects the fix: `LOAD_FAILED` ⇒ a format/URI bug (`UIImage(contentsOfFile:)` → `nil` → `INVALID_IMAGE`), `MISS` ⇒ the file is marginal for Vision, `HIT` ⇒ Vision reads it and the app does not, which is the only case justifying a device session and deep analysis of the library's Swift. A real iOS device (never the simulator — it pins an older Vision revision) then confirms whether `scan` resolved `[]` or rejected, and whether the **iOS camera** reads the physical card (AVFoundation ≠ Vision).
...
- **The iOS gap is closed or explicitly deferred with evidence**, on the branch the harness selected: a marginal file gets a **crop-and-retry** recovery action (`allowsEditing: true` on the retry only, never the default — no new dependency, OTA-safe); a load failure gets fixed where the URI/format is produced; a Vision-reads-it-but-we-don't result opens the library's Swift, a spike on `expo-camera`'s exported-but-unused `scanFromURLAsync`, and an upstream report plus a CI-guarded `yarn patch` if warranted; and a documented deferral is acceptable with the numbers to back it. On every branch, the manual-entry escape hatch must be reachable and obvious from the failure state.
