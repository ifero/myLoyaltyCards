...
- **Auto-trigger + long-running manual sync race** (QA round 1 low) — ✅ **RESOLVED in the efficiency pass** (the shared in-flight guard made it cross-instance and more reachable, so it was fixed rather than deferred). `runAutoSync` now checks the shared `isRunning` guard _inside_ the retry callback: if a manual sync already owns it, the auto path bails cleanly (no retry-budget burn, no banner) and leaves the latch unset so a later event can re-fire. Covered by `race-safety › auto-sync started while a manual sync is in flight bails quietly`.
