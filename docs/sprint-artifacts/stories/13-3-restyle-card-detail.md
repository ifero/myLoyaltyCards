...
- [ ] Catalogue cards (`card.brandId !== null`) display a brand-colored header using the brand's hex color from `catalogue/data/italy.json`
- [ ] Catalogue cards show the brand logo centered in the hero (sourced via `useBrandLogo` hook)
- [ ] Brand name is displayed prominently below the hero header
- [ ] Custom cards (`card.brandId === null`) display the user-selected `card.color` as the hero background
- [ ] Custom cards show a first-letter avatar (via `VirtualLogo` or equivalent) centered in the hero
- [ ] Hero section matches Figma frames: "Catalogue Card Detail" and "Custom Card Detail"
- [ ] Dark mode: hero colors remain the same (brand colors are constant), text contrast maintained

### AC2: Barcode Display Area

- [ ] Barcode is rendered large and clear inside a white/light card container
- [ ] White barcode background is maintained even in dark mode (scanner readability)
- [ ] Barcode number displayed below in a readable, spaced format (e.g., `1234 5678 9012`)
- [ ] "Tap to enlarge" hint text shown below barcode area
- [ ] Tapping barcode area opens fullscreen barcode overlay (AC6)
- [ ] Barcode card has proper padding and subtle shadow/elevation for visual separation
- [ ] Matches Figma frame: barcode section within Card Detail

### AC3: Card Info Section

- [ ] Info section displays metadata in labeled rows:
  ...
  - Date added
- [ ] Barcode format row is NOT displayed (removed per design review -- implementation detail, not user-relevant)
- [ ] Copy-to-clipboard triggers haptic feedback and shows a toast confirmation via `burnt`
- [ ] Info section is visually secondary to the barcode (smaller type, muted labels)
- [ ] Dark mode: surface color for info card, text colors follow semantic tokens

### AC4: Manage Actions Section

- [ ] "Manage" section header label displayed above action rows
- [ ] "Edit card" action row using shared `ActionRow` component: MI: edit icon + "Edit card" label + MI: chevron-right
- [ ] "Delete card" action row: MI: delete icon + "Delete card" label, red/destructive text color, positioned LAST
- [ ] Delete row is visually de-emphasized compared to Edit (no chevron, quiet destructive style)
- [ ] Visual separator between Edit and Delete rows
- [ ] Tapping Edit navigates to `/card/[id]/edit`
- [ ] Tapping Delete shows confirmation `Alert.alert` dialog (existing behavior preserved)
- [ ] 44pt minimum touch target on all action rows
- [ ] Dark mode: action rows use dark surface, destructive red remains visible

### AC5: Navigation Header

- [ ] Header shows MI: chevron-left back arrow on the left
- [ ] Card name centered in header
- [ ] Header uses brand color as background (catalogue) or primary color (custom)
- [ ] Header text is white (or contrast-appropriate for the brand color)
- [ ] On scroll, header condenses (smaller title, more compact)
- [ ] Back arrow navigates to previous screen
- [ ] Matches Figma frame: navigation header in Card Detail

### AC6: Fullscreen Barcode Overlay

- [ ] Tapping barcode area opens a fullscreen barcode modal/overlay
- [ ] Fullscreen overlay uses white background even in dark mode (scanner readability)
- [ ] MI: close icon in top-right (or top-left) to dismiss the overlay
- [ ] Barcode is rendered at maximum width for the screen
- [ ] Barcode number shown below in spaced format
- [ ] Screen brightness hint displayed (MI: light-mode icon + text)
- [ ] Overlay is dismissible via close icon tap
- [ ] Matches Figma frame: "Barcode enlarged/fullscreen state"

### AC7: Screen Brightness Hint

- [ ] A brightness hint (MI: light-mode icon + "Increase brightness for scanning") is visible on the detail screen near the barcode area
- [ ] Hint uses secondary/muted text styling
- [ ] Existing `useBrightness` hook behavior preserved (auto-brightness on fullscreen)

### AC8: Dark Mode Support

- [ ] White background in light mode, true black `#000000` background in dark mode
- [ ] All text follows semantic color tokens (textPrimary, textSecondary)
- [ ] Barcode area remains white in both modes
- [ ] Brand hero colors are consistent across light/dark
- [ ] Action rows use appropriate dark surface colors
- [ ] Destructive red text meets contrast requirements on dark surfaces
- [ ] Matches Figma dark frames for all four Card Detail states

### AC9: Old Code Cleanup

- [ ] All `SAGE_COLORS` references removed from `CardDetails.tsx` and `app/card/[id].tsx`
- [ ] Emoji clipboard icon (`clipboard`) replaced with MI: content-copy vector icon
- [ ] Inline `StyleSheet.create` styles migrated to NativeWind classes where appropriate (layout/spacing)
- [ ] Dynamic theme colors applied via `useTheme()` tokens from 13-1

### AC10: Tests Pass

- [ ] All new and modified component tests pass
