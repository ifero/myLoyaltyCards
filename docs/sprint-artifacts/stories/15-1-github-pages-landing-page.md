...
- [ ] **Screenshots** — row of 2-3 device frame placeholders (phone screens). Each placeholder is a labelled SVG with dashed border and centred text "Screenshot: {screen name}". `<!-- SCREENSHOT: {name} -->` comment above each for easy replacement.
- [ ] **Footer** — GitHub repo link, "Open Source — MIT License" badge/text, privacy policy link (`/privacy-policy` — can be a placeholder `#` for now)
...
*Italian:* "Le tue carte fedeltà. Sempre al polso."
